...
# Constants
ROOM_COST_PER_DAY = 500

# Initialize available rooms
available_rooms = list(range(1, 6))
...
class HotelManagement:
    def __init__(self, root):
        ...
        # Create buttons
        btn_fetch_rooms = tk.Button(root, text="Fetch Available Room Data", bg="yellow", fg="black",font="bold", command=self.fetch_available_rooms)
        btn_check_in = tk.Button(root, text="Check In",bg="yellow", fg="black",font="bold", command=self.check_in)
        btn_check_out = tk.Button(root, text="Check Out", bg="yellow", fg="black",font="bold",command=self.check_out)
        btn_show_guest_list = tk.Button(root, text="Show All Guest List",bg="yellow", fg="black",font="bold", command=self.show_guest_list)
        btn_show_history = tk.Button(root, text="Show Guest History",bg="yellow", fg="black",font="bold", command=self.show_guest_history)
    ...
    def show_guest_list(self):
        try:
            con = pymysql.connect(host='localhost', user='root', passwd='rahul', database='hotel')
            cur = con.cursor()
